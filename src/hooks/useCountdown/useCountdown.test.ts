...
describe('useCountDown()', () => {
  test('should use countdown', () => {
    const { result } = renderHook(() => useCountdown({ seconds: 60, interval: 500, isIncrement: false }));
...
describe('start', () => {
  test('should increment count', () => {
    const { result } = renderHook(() => useCountdown({ seconds: 60, interval: 500, isIncrement: true }));
