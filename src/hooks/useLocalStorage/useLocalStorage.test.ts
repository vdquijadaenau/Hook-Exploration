...
describe('useLocalStorage()', () => {
  ...
  test('Update the state writes to locastorage', () => {
    const { result } = renderHook(() => useLocalStorage('key', 'value'));
  ...
  test('Update the state with undefines', () => {
    const { result } = renderHook(() => useLocalStorage<string | undefined>('keytest', 'value'));
    act(() => {
      const setState = result.current[1];
      setState(undefined);
    });
  ...
  test('[Event] Updating one hook update the others', () => {
