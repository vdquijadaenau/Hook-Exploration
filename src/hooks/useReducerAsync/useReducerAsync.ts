import { useLayoutEffect, useEffect, useRef, Reducer, Dispatch, ReducerAction, ReducerState, useReducer, useCallback } from 'react';
...
export type AsyncActionHandlers<R extends Reducer<any, any>, AsyncAction extends { type: string }> = {
...
>(reducer: R, initialiserArg: I, initialiser: (arg: I) => ReducerState<R>): [ReducerState<R>, Dispatch<ExportAction>];
...
>(
  ...
  const aaHandlers = (asyncActionHandlers || initializer) as AsyncActionHandlers<R, AsyncAction>;
  const [state, dispatch] = useReducer(reducer, initializerArg as any, asyncActionHandlers && (initializer as any));
  ...
  const wrappedDispatch = useCallback(
    (action: AsyncAction | ReducerAction<R>) => {
      ...
      const aaHandler = ((type && aaHandlers[type]) || null) as typeof action extends AsyncAction
        ? (s: {
            dispatch: Dispatch<AsyncAction | ReducerAction<R>>;
            getState: () => ReducerState<R>;
            signal: AbortSignal;
