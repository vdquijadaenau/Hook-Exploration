...
import useMap, { MapEntries } from './useMap';

describe('useMap()', () => {
  it('it should be initiated with a map', () => {
    const initialMap = new Map([[1, 'initial']]);
    const { result } = renderHook(() => useMap(initialMap));
    expect(result.current[0].get(1)).toBe('initial');
  });

  it('it shoulb be initiate with an array', () => {
    const initialArray: MapEntries<number, string> = [[1, 'initial']];
    const { result } = renderHook(() => useMap(initialArray));
    expect(result.current[0].get(1)).toBe('initial');
  });
  it('it should initiate empty', () => {
    const { result } = renderHook(() => useMap());
    expect(result.current[0].size).toBe(0);
  });
  it('it should add a new value', () => {
  ...
  it('it should update existing values', () => {
    const initialMap = new Map([[1, 'initial']]);
    const { result } = renderHook(() => useMap(initialMap));
  ...
  it('should setAll replace all exisiting values', () => {
