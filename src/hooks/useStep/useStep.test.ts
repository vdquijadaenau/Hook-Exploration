...
describe('useStep()', () => {
  ...
  test('should decrement step', () => {
    ...
    act(() => result.current[1].setStep(2));
  ...
  test('should set step', () => {
    ...
    const newStep = 2;
    act(() => result.current[1].setStep(newStep));

    expect(result.current[0]).toBe(newStep);
  });

  test('should return is prev step is available', () => {
    const { result } = renderHook(() => useStep(2));
    ...
    act(() => result.current[1].setStep(1));
    expect(result.current[1].canGoToPrevStep).toBe(false);
  });

  test('should return i next step is available', () => {
    const { result } = renderHook(() => useStep(2));
